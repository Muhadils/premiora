...
import { Plus, X, UploadCloud, CheckCircle2, CloudDownload, Pencil } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { CATEGORIES } from "@/lib/utils/constants";
import { updateProduct, uploadImage } from "@/lib/actions/product.actions";
...
export function EditProductModal({ product }: { product: any }) {
  ...
  useEffect(() => {
    if (product && isOpen) {
      ...
      let tText = "";
      if (desc.includes("||SNK_TEXT:")) {
        const parts = desc.split("||SNK_TEXT:");
        desc = parts[0];
        tText = parts[1]?.trim();
      } else if (desc.includes("||SNK:")) {
        const parts = desc.split("||SNK:");
        desc = parts[0];
        tText = parts[1]?.trim();
      }
  ...
  const resetForm = () => {
    ...
    setImageSrc(null);
    setCroppedBlob(null);
    setForm({ name: "", category: "", price: "", supplier_product_code: "", image_url: "", description: "", terms_text: "" });
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const finalDescription = form.terms_text ? `${form.description}||SNK_TEXT:${form.terms_text}` : form.description;
    ...
    startTransition(async () => {
      try {
        ...
        // Jika user melakukan crop gambar, unggah dulu ke Supabase
        if (croppedBlob) {
          const uploadData = new FormData();
          uploadData.append("file", croppedBlob, "product_image.jpg");
  ...
  return (
    <>
      <Button onClick={() => setIsOpen(true)} variant="outline" size="sm" className="gap-2 text-primary-600 hover:text-primary-700 w-full sm:w-auto">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <AnimatePresence mode="wait">
              {isSuccess ? (
                ...
                >
                  ...
                  <p className="text-slate-500">Perubahan berhasil disimpan.</p>
                </motion.div>
              ) : (
                <motion.div
                  key="form"
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h2 className="text-xl font-bold text-slate-900">Edit Produk</h2>
              <button 
...
- Pakai aplikasi Capcut versi terbaru..." 
      ...
      {/* Cropper Modal (muncul di atas modal edit product) */}
      {imageSrc && (
        <ImageCropper 
          imageSrc={imageSrc} 
