...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 text-sm font-medium text-slate-900 transition-all hover:text-primary-600 [&[data-state=open]>svg]:rotate-180 cursor-pointer text-left",
        className
      )}
      {...props}
    >
